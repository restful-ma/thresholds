...
function main() {
    ...
    switch (inputArg) {
        ...
        default: {
            console.log("Invalid argument! Please use openapi, raml, or wadl as argument");
            return;
        }
    }
}
...
async function executeJar(files, inFolder, outFolder, format) {
    ...
    for (let i = 0; i < files.length; i++) {
        try {
            ...
            await getJsonMetrics(files[i], inFolder, outFolder, format).catch(err => {
                // failure because the cli console output (exceeds buffer), but execution was still valid
                if (err.code === "ERR_CHILD_PROCESS_STDIO_MAXBUFFER") {
                    console.log("CLI output exceeds buffer, but still valid. Just in case this is logged to MetricOutputTooBig.txt");
                    fs.appendFileSync("MetricOutputTooBig.txt", files[i] + "\n");
                } else {
                    console.log("SKIPPED " + files[i]);
                    console.log("LOG ERROR to error.log----------------------------------------------------");
                    console.log(err);
...
function elapsedTime() {
    ...
    console.log(`Total time in seconds: ${process.hrtime(start)[0]}s, time for last file: ${elapsed}ms`);
}
