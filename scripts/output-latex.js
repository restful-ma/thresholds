...
const RESULTS_JSON = "./eval/results.json";
...
for (let i = 0; i < metricsNames.length; i++) {
    ...
    const latexTable =
        ...
        + round(metrics[metricName]["all"]["25%"]) + " \\\\ \\hline\n "
        // Q2-Median
        ...
        + round(metrics[metricName]["all"]["50%"]) + " \\\\ \\hline\n "
        // Q3
        ...
        + round(metrics[metricName]["all"]["75%"]) + " \\\\ \\hline\n "
        // Number of Files
        + "    Total Files &"
        + formats["raml"] + "&"
        + formats["wadl"] + "&"
        + formats["openapi"] + "&"
        + (parseInt(formats["raml"]) + parseInt(formats["wadl"]) + parseInt(formats["openapi"])) + " \\\\ \\hline\n "
